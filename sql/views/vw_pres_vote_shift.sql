-- drop view vw_pres_vote_shift
create or replace view vw_pres_vote_shift
as
    with vote as (
        select 
            election
    ...
    select 
        ...
        ,odiff
        ,dem_raw_margin
        ,dem_raw_margin - lag(dem_raw_margin, 1) OVER (partition by fips order by election asc) dem_raw_margin_shift
        ,dem_percent
        ,rep_percent
        ,oth_percent
